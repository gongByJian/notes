...
##### [常用Maven插件介绍](https://www.cnblogs.com/jqmtony/p/7258511.html)

我们都知道Maven本质上是一个插件框架，它的核心并不执行任何具体的构建任务，所有 这些任务都交给插件来完成，例如编译源代码是由maven- compiler-plugin完成的。进一步说，每个任务对应了一个插件目标（goal），每个插件会有一个或者多个目标，例如maven- compiler-plugin的compile目标用来编译位于`src/main/java/`目录下的主源码，testCompile目标用来编译位于`src/test/java/`目录下的测试源码。
